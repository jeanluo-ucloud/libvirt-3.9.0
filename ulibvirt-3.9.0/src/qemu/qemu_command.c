                         ...
                         qemuDomainSecretInfoPtr secinfo)
{
    ...
    switch ((virStorageNetProtocol) src->protocol) {
        ...
        case VIR_STORAGE_NET_PROTOCOL_UDISK:
            if (src->nhosts == 0) {
                if (virAsprintf(&ret, "udisk:%s", src->path) < 0)
                    goto cleanup;
            } else {
                /* only one host is supported now */
                if (virAsprintf(&ret, "udisk:%s:%d:%s",
                            src->hosts->name, src->hosts->port, src->path) < 0)
                    goto cleanup;
            }
            break;
                          ...
                          int idx)
{
    ...
    /*
     * We have VM accidentally use io=native && cache=writethrough in xml.
     * Even though this is a wrong configuration, we move on.
     */
#if 0
                   ...
                   virQEMUCapsPtr qemuCaps)
{
    ...
    if (usingVirtio) {
        ...
        if (net->driver.virtio.host.standby) {
            virBufferAsprintf(&buf, ",standby=%s",
                              virTristateSwitchTypeToString(net->driver.virtio.host.standby));
        }
                        ...
                        virQEMUCapsPtr qemuCaps)
{
    ...
    if (def->features[VIR_DOMAIN_FEATURE_QEMU_CPU] == VIR_TRISTATE_SWITCH_ON) {
        for (i = 0; i < VIR_DOMAIN_QEMU_CPU_LAST; i++) {
            switch ((virDomainQemuCpu) i) {
            case VIR_DOMAIN_QEMU_CPU_LEGACY_CACHE:
                if (def->qemu_cpu_features[i] == VIR_TRISTATE_SWITCH_ON ||
                    def->qemu_cpu_features[i] == VIR_TRISTATE_SWITCH_OFF)
                    virBufferAsprintf(&buf, ",legacy-cache=%s",
                                      virTristateSwitchTypeToString(def->qemu_cpu_features[i]));
                break;

            case VIR_DOMAIN_QEMU_CPU_XLEVEL:
                if (def->qemu_cpu_features[i] == VIR_TRISTATE_SWITCH_ON)
                    virBufferAsprintf(&buf, ",xlevel=0x%x",
                                      def->qemu_cpu_xlevel);
                break;

            /* coverity[dead_error_begin] */
            case VIR_DOMAIN_QEMU_CPU_LAST:
                break;
            }
        }
    }
                              ...
                              bool chardevStdioLogd)
{
    ...
    */
    ...
    if (net->txqueuelen &&
        virNetDevSetTXQ(net->ifname, net->txqueuelen) < 0)
        goto cleanup;
...
char *
qemuBuildVhostUserChardevStr(virDomainVhostUserDefPtr vhostuser)
{
    virBuffer buf = VIR_BUFFER_INITIALIZER;

    virBufferAsprintf(&buf, "socket,id=%s,path=%s,reconnect=%d", 
			vhostuser->id, vhostuser->path, vhostuser->reconnect);
    return virBufferContentAndReset(&buf);
}

char *
qemuBuildVhostUserDeviceStr(virDomainVhostUserDefPtr vhostuser,
                            virDomainDefPtr def,
                            virQEMUCapsPtr qemuCaps)
{
    virBuffer buf = VIR_BUFFER_INITIALIZER;

    virBufferAsprintf(&buf, "vhost-user-blk-pci,id=%s,chardev=%s,num-queues=%d", 
			vhostuser->id, vhostuser->id, vhostuser->queues);
...
qemuBuildVhostUserCommandLine(virCommandPtr cmd,
                              virDomainDefPtr def,
                              virDomainVhostUserDefPtr vhostuser,
                              virQEMUCapsPtr qemuCaps)
{
    char *devstr = NULL;
    
    devstr = qemuBuildVhostUserChardevStr(vhostuser);
    if (devstr == NULL)
        return -1;
    virCommandAddArgList(cmd, "-chardev", devstr, NULL);
    VIR_FREE(devstr);

    devstr = qemuBuildVhostUserDeviceStr(vhostuser, def, qemuCaps);
    if (devstr == NULL)
        return -1;
    virCommandAddArgList(cmd, "-device", devstr, NULL);
    VIR_FREE(devstr);
...
static int
qemuBuildVhostUserMemCommandLine(virCommandPtr cmd,
                          virDomainVhostUserMemDefPtr vhostusermem)
{
    char *devstr = NULL;
    virBuffer buf = VIR_BUFFER_INITIALIZER;

    if (vhostusermem == NULL)
	return 0;

    virBufferAsprintf(&buf, "memory-backend-file,id=%s,size=%s,mem-path=%s,share=on,align=%s", 
			vhostusermem->id, vhostusermem->size, vhostusermem->path, vhostusermem->align);
    devstr = virBufferContentAndReset(&buf);
    virCommandAddArgList(cmd, "-object", devstr, NULL);
    VIR_FREE(devstr);

    virBufferAsprintf(&buf, "node,memdev=%s", vhostusermem->id);
    devstr = virBufferContentAndReset(&buf);
    virCommandAddArgList(cmd, "-numa", devstr, NULL);
    VIR_FREE(devstr);
                     ...
                     int **nicindexes)
{
    ...
    for (i = 0; i < def->nvhostusers; i++) {
        if (qemuBuildVhostUserCommandLine(cmd, def, def->vhostusers[i], qemuCaps) < 0)
            goto error;
    }

    if (qemuBuildVhostUserMemCommandLine(cmd, def->vhostusermem))
        goto error;
