...
static void vhost_user_blk_start(VirtIODevice *vdev)
{
    ...
    ret = vhost_recovery_start(&s->dev, s->recovery_pool, s->num_queues);
    if (ret < 0) {
        error_report("Error starting recovery: %d", -ret);
        goto err_guest_notifiers;
    }
...
static void vhost_user_blk_stop(VirtIODevice *vdev)
{
    ...
    s->vhost_user_blk_started = false;
}
...
static void vhost_user_blk_set_status(VirtIODevice *vdev, uint8_t status)
{
    ...
    if (should_start) {
        info_report("set status start");
        vhost_user_blk_start(vdev);
    } else {
        info_report("set status stop");
...
static void vhost_user_blk_handle_output(VirtIODevice *vdev, VirtQueue *vq)
{
    ...
    if (s->vhost_user_blk_started == true) {
        return;
    }
...
static int vhost_user_blk_reconnect(VHostUserBlk *s)
{
    ...
    ret = vhost_dev_reconnect(&s->dev, &s->chardev, VHOST_BACKEND_TYPE_USER, 0);
    if (ret != 0) {
    ...
    if (s->dev.started == true) {
        vhost_user_blk_stop(&s->parent_obj);
    }

    vhost_user_blk_start(&s->parent_obj);
...
static void vhost_user_blk_event(void *opaque, int event) 
{
    ...
    case CHR_EVENT_OPENED:
        info_report("vhost_user_blk_event OPENED");
        s->watch = qemu_chr_fe_add_watch(&s->chardev, G_IO_HUP, vhost_user_blk_watch, s);
        if (vhost_user_blk_reconnect(s) != 0) {
            error_report("vhost_user_blk_reconnect failed");
            return ;
	}
        //qemu_chr_fe_set_handlers(&s->chardev, NULL, NULL, NULL, NULL, NULL, NULL, false);
        info_report("vhost_user_blk_event end OPENED");
        break;
    case CHR_EVENT_CLOSED:
        info_report("vhost_user_blk_event CLOSED");
#if 0
        if (s->watch != 0) {
...
#endif
        qemu_chr_fe_set_handlers(&s->chardev, NULL, NULL, NULL, NULL, NULL, NULL, false);
        info_report("vhost_user_blk_event end CLOSED");
        break;
    default:
        ...
        void *opaque)
{
    ...
    info_report("vhost_user_blk_watch diskconnect:%p", s->dev.vhost_ops);
...
static void vhost_user_blk_device_realize(DeviceState *dev, Error **errp)
{
    ...
    ret = vhost_recovery_init(&s->recovery_pool, s->num_queues);
    if (ret < 0) {
        error_setg(errp, "vhost-user-blk: vhost recovery initialization failed: %s",
                   strerror(-ret));
        goto vhost_err;
    }
    ...
    return;
recovery_err:
    vhost_recovery_destroy(&s->recovery_pool, s->num_queues);
vhost_err:
...
static Property vhost_user_blk_properties[] = {
    ...
    DEFINE_PROP_BIT("config-wce", VHostUserBlk, config_wce, 0, false),
