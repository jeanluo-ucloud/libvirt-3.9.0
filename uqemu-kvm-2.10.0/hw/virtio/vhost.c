...
static int vhost_recovery_shm_vq_init(struct vhost_recovery_shm *pool, int vq)
{
    Error *err = NULL;
    ...
    snprintf(name, sizeof(name), "vhost-recovery-vq-%d", vq);
    shm->shm_addr = qemu_memfd_alloc(name, VHOST_RECOVERY_MEM_SIZE, 0,
                                    &shm->shm_fd, &err);
    if (err) {
    ...
    info_report("qemu_memfd_alloc shm_addr:%p, shm->shm_fd:%d, vq:%d\n", shm->shm_addr, shm->shm_fd, vq);

    memset(shm->shm_addr, 0, VHOST_RECOVERY_MEM_SIZE); 
    
    return 0; 
}

static void vhost_recovery_shm_vq_destroy(struct vhost_recovery_shm *pool, int vq)
{
    struct vhost_recovery_shm * shm; 
    ...
    if (shm->shm_addr) {
        info_report("qemu_memfd_free shm_addr:%p, shm->shm_fd:%d, vq:%d\n", shm->shm_addr, shm->shm_fd, vq);
        qemu_memfd_free(shm->shm_addr, VHOST_RECOVERY_MEM_SIZE, shm->shm_fd);
        shm->shm_addr = NULL;
        shm->shm_fd = -1;
    }
}

int vhost_recovery_init(struct vhost_recovery_shm **pool, int vq_size)
{
    int i, r;
    ...
    *pool = g_malloc0(sizeof (struct vhost_recovery_shm) * vq_size);
    if (pool == NULL) {
    ...
    for (i = 0; i < vq_size; i++) {
        r = vhost_recovery_shm_vq_init(*pool, i);
        if (r < 0) {
            goto fail;
        }
    }
...
fail:
    i--;
    while(i >= 0) {
        vhost_recovery_shm_vq_destroy(*pool, i);
	i--;
    }
...
void vhost_recovery_destroy(struct vhost_recovery_shm **pool, int vq_size)
{
    int i;
...
int vhost_recovery_start(struct vhost_dev *dev, 
                              struct vhost_recovery_shm *pool, 
                              int vq_size)
{
    int i, r;
    ...
    for (i = 0; i < vq_size; i++) {
       file.index = i;
       file.fd = pool[i].shm_fd;
       r = dev->vhost_ops->vhost_set_recovery_fd(dev, &file);
       if (r < 0) {
           error_report("vhost_ops->set_recovery_fd failed");
           return -1;
       }
    } 
...
int vhost_dev_reconnect(struct vhost_dev *hdev, void *opaque,
                   VhostBackendType backend_type, uint32_t busyloop_timeout)
{
    uint64_t features;
    ...
    hdev->log_enabled = false;
    hdev->started = false;
    vhost_migration_log(&hdev->memory_listener, true);
#endif
