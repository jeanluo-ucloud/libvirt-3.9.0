                            ...
                            int flags, Error **errp)
{
    ...
    if (s->is_shared_disk && (s->mode == REPLICATION_MODE_PRIMARY)) {
        ...
        tmp_bs = bdrv_lookup_bs(s->shared_disk_id, s->shared_disk_id, NULL);
        if (!tmp_bs || !tmp_bs->file) {
            error_setg(&local_err, "No share_disk_id %s, or it is invalid", s->shared_disk_id);
            goto fail;
        }
...
static void replication_close(BlockDriverState *bs)
{
    ...
    if (s->stage == BLOCK_REPLICATION_FAILOVER && s->active_disk->bs->job) {
...
static void replication_done(void *opaque, int ret)
{
    ...
    if (ret == 0) {
        ...
        /*
         * replace replication top_bs with secondary_disk's bs, that way we can
         * delete replication from block graph.
         */
        bdrv_replace_node(bs, s->secondary_disk->bs, &local_err);
        if (local_err) {
