                       ...
                       void *opaque, int version_id)
{
    ...
    while (field->name) {
        ...
        if ((field->field_exists &&
            ...
            /* in qemu 0.12, xsave cpu feature in subsection, and move to
             * main section in latter version, this introduce a migration issue,
             * so we load it in the subsection if from qemu 0.12.
             */
            if (!strcmp(field->name, "env.xcr0")) {
                char *buffer, fieldname[256];
                size_t len;

                len = strlen("cpu/xsave");
                if (len &&
                    qemu_peek_buffer(f, (uint8_t **)&buffer, len, 2) == len) {
                    memcpy(fieldname, buffer, len);
                    fieldname[len] = 0;
                    if (!strcmp(fieldname, "cpu/xsave")) {
                        fprintf(stderr, "next field: xsave, migrate from a legacy version?\n");
                        break;
                    }
                }
                ...
                len = strlen("apic");
                if (len &&
                    qemu_peek_buffer(f, (uint8_t **)&buffer, len, 6) == len) {
                    memcpy(fieldname, buffer, len);
                    fieldname[len] = 0;
                    if (!strcmp(fieldname, "apic")) {
                        fprintf(stderr, "next field: apic, migrate from a legacy version?\n");
                        break;
                    }
                }
            }
