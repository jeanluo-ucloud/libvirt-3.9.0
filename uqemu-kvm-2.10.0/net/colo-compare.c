...
static void colo_compare_handle_event(void *opaque)
{
    ...
    case COLO_EVENT_CHECKPOINT:
        ...
        if (g_hash_table_size(s->connection_track_table) > HASHTABLE_MAX_SIZE) {
            trace_colo_compare_main("colo compare connection hashtable full,"
                                  " clear it");
            connection_hashtable_reset(s->connection_track_table);
            /*
             * clear the conn_list
             */
            while (!g_queue_is_empty(&s->conn_list)) {
                connection_destroy(g_queue_pop_head(&s->conn_list));
            }
        }
        break;
...
static void colo_compare_iothread(CompareState *s)
{
    ...
    s->event_bh = aio_bh_new(iothread_get_aio_context(s->iothread), colo_compare_handle_event, s);
}
